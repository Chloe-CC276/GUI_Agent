...
export function ERPPOCreatePage() {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const data = await api.getPOCreateContext(taskId)
      setCtx(data)
      ...
      if (data.task_id && data.status === 'QUEUED') {
        await api.runPOTask(data.task_id)
      }
    } catch (e) {
  ...
  const createPo = async () => {
    ...
    try {
      setSaving(true)
      const result = await api.createPOFromForm(ctx.task_id, {
        header,
    ...
    <div className="erp-classic-shell" data-testid="erp-po-create-page">
      <div className="page-title">
        <div>
          ...
          <Typography.Text type="secondary">ME21N 风格建单页 · Agent / 人工均可录入</Typography.Text>
        </div>
        <Space>
      ...
      <Alert
        ...
        showIcon
        message="方案 A 说明"
        description="本页是 ERP 本系统建单界面。采购云不通过业务 API 直推 PO；由 GUI Agent 在此页面填写并保存。"
      />
      ...
      <Card title="PO Header" className="section-card erp-classic-card" data-testid="erp-po-create-form">
        <div className="erp-form-grid">
          <label>供应商名称<input data-testid="erp-po-supplier" value={header.supplier_name || ''} onChange={(e) => setHeader({ ...header, supplier_name: e.target.value })} /></label>
          <label>供应商编码<input data-testid="erp-po-supplier-code" value={header.supplier_code || ''} onChange={(e) => setHeader({ ...header, supplier_code: e.target.value })} /></label>
          <label>采购组织<input data-testid="erp-po-purchasing-org" value={header.purchasing_org || ''} onChange={(e) => setHeader({ ...header, purchasing_org: e.target.value })} /></label>
          <label>采购组<input data-testid="erp-po-purchasing-group" value={header.purchasing_group || ''} onChange={(e) => setHeader({ ...header, purchasing_group: e.target.value })} /></label>
          <label>币种<input data-testid="erp-po-currency" value={header.currency_code || ''} onChange={(e) => setHeader({ ...header, currency_code: e.target.value })} /></label>
          <label>付款条件<input data-testid="erp-po-payment-terms" value={header.payment_terms || ''} onChange={(e) => setHeader({ ...header, payment_terms: e.target.value })} /></label>
          <label>需求部门<input data-testid="erp-po-request-dept" value={header.request_dept || ''} onChange={(e) => setHeader({ ...header, request_dept: e.target.value })} /></label>
          <label>采购员<input data-testid="erp-po-buyer" value={header.buyer_id || ''} onChange={(e) => setHeader({ ...header, buyer_id: e.target.value })} /></label>
        </div>
